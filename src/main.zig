...
pub fn main() !void {
    ...
    const file = try std.fs.cwd().createFile("output.xml", .{});
    defer file.close();
...
fn getFilters(vf: *VideoFile) !struct { fg: *av.FilterGraph, abuffer: *av.FilterContext, abuffersink: *av.FilterContext } {
    ...
    try silencedetect.init_str("n=-30dB:d=2");
...
fn fcxml_1_8_addclip(allocator: std.mem.Allocator, offset: f64, start: f64, duration: f64) ![]const u8 {
    ...
    var writer = buffer.writer();
    // "                        "
    try writer.print("                        <asset-clip ref=\"r2\" {s} name=\"{s}\" {s} {s} tcFormat=\"NDF\" audioRole=\"dialogue\"/>\n", .{
        buff_offset,
        video_file.name(),
        buff_start,
        buff_duration,
    ...
    if (extractValue(message)) |sevent| {
        switch (sevent.type) {
            0 => {
                ...
                const dialogue_duration = dialogue_end - dialogue_start;
                if (dialogue_duration == 0.0) return;

                cur_clip_index += 1;
                const clip = fcxml_1_8_addclip(fcpxml_allocator, cur_offset, dialogue_start, dialogue_duration) catch |err| {
                    std.debug.print("Error al agregar clip: {}\n", .{err});
                    return;
                };

                fcpxml_lines.append(clip) catch |err| {
                    std.debug.print("Error al agregar clip: {}\n", .{err});
                    return;
                };
